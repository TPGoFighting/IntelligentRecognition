...
# ✅ 注意：这里严禁写 from models.pointnet2_sem_seg import ... 否则会循环报错

def square_distance(src, dst):
    """计算平方距离，并增加 clamp 防止梯度爆炸导致的 nan"""
    B, N, _ = src.shape
    _, M, _ = dst.shape
...
def farthest_point_sample(xyz, npoint):
    ...
    if npoint > N:  # 安全补丁：点数不足时补齐
        idx = torch.arange(N, dtype=torch.long).to(device).view(1, N).repeat(B, 1)
        pad = torch.randint(0, N, (B, npoint - N), dtype=torch.long).to(device)
...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        if S == 1:
            ...
            weight = dist_recip / (norm + 1e-10)
            interpolated_points = torch.sum(index_points(points2.permute(0, 2, 1), idx) * weight.view(B, N, 3, 1),
                                            dim=2)
